...
static esp_err_t wifi_page_handler(httpd_req_t *req)
{
  ...
  html += "<p><strong>Saved Hostname:</strong> " + (saved_hostname.length() ? saved_hostname : String("(none)")) + "</p>";
  html += "<p><strong>mDNS Name:</strong> " + (mdns_name.length() ? mdns_name : String("(not configured)")) + "</p>";
  html += "<p><strong>STA Connected:</strong> " + String(CameraWebServerWS.IsStationConnected() ? "yes" : "no") + "</p>";
  html += "<p><strong>STA IP:</strong> " + CameraWebServerWS.GetStationIP() + "</p></div>";
  ...
  html += "<input name='hostname' placeholder='Optional hostname, e.g. hiro1' maxlength='63' pattern='[a-z0-9-]+' value='" + saved_hostname + "'>";
  html += "<button type='submit'>Save And Reboot</button></form>";
  html += "<p>This stores credentials in NVS, reboots, tries STA first, and falls back to AP if join fails.</p>";
  html += "<p>Hostnames are optional and should be unique within your classroom, for example <code>hiro1</code>.</p></div>";
...
{
  String json = "{";
  json += "\"mode\":\"" + CameraWebServerWS.GetModeLabel() + "\",";
  json += "\"ap_ssid\":\"" + CameraWebServerWS.GetActiveAccessPointSSID() + "\",";
  ...
  json += "\"mdns_name\":\"" + CameraWebServerWS.GetMdnsName() + "\"}";
...
static esp_err_t wifi_config_handler(httpd_req_t *req)
{
  ...
  String station_hostname = form_value(form, "hostname");
  ...
  if (station_hostname.endsWith(".local"))
  {
    station_hostname.remove(station_hostname.length() - 6);
  }

  if (station_hostname.length() > 0 && !CameraWebServerWS.IsValidHostnameLabel(station_hostname))
  {
    httpd_resp_send_err(req, HTTPD_400_BAD_REQUEST, "hostname must use lowercase letters, numbers, and hyphens only");
    return ESP_FAIL;
  }

  if (!CameraWebServerWS.SaveStationCredentials(station_ssid, station_password, station_hostname))
  {
  ...
  if (station_hostname.length() > 0)
  {
    html += "<p>Saved hostname <strong>" + station_hostname + ".local</strong>.</p>";
  }
